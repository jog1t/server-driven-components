# useReactive - Reactive Server Components API

Simple, React-like API for reactive server components with automatic SSE streaming.
...
├─────────────────────────────────────────────────────────┤
│                                                          │
│  Component with useReactive()                           │
│         │                                                │
│         ▼                                                │
│  Reactive Runtime                                       │
│  ├─ Register stream with deps                          │
│  ├─ Deduplicate by scope                               │
│  └─ Start/stop stream lifecycle                        │
│         │                                                │
│         ▼                                                │
│  Stream produces updates                               │
│         │                                                │
│         ▼                                                │
│  Component rendered to RSC payload                     │
│         │                                                │
│         ▼                                                │
│  SSE endpoint streams chunks                           │
│                                                          │
└──────────────────┬──────────────────────────────────────┘
                   │ SSE (text/event-stream)
                   │ RSC payload chunks (base64)
                   ▼
┌─────────────────────────────────────────────────────────┐
│                     Client Side                         │
├─────────────────────────────────────────────────────────┤
...
- Reactive state lives on server (not persisted by default)
- No client-side interactivity within reactive components (use client components for that)
- SSE connection required (fallback to polling not implemented)
- Single server only (multi-server would need Redis/external state)

## Next Steps

- [ ] Add persistence layer for reactive state
- [ ] Implement computed signals with reactive graph
- [ ] Add dev tools for debugging streams
- [ ] Support server actions in reactive components
- [ ] Selective streaming (only send diffs)
- [ ] Multi-server support with Redis backend
